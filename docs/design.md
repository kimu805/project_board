...
- 案件カードに以下を表示：案件名、クライアント名（公開設定時のみ）、単価、勤務形態、期間、技術スタック
- ステータスに応じた色分けインジケーター（緑: 参画中、黄: 参画前、灰: 終了）
- ホバー時に詳細・編集・削除ボタンを表示

#### 案件詳細画面（show）
- パンくずナビゲーション
- 左カラム：案件名、クライアント名（公開設定時のみ）、技術スタック、参画期間
...
| name        : STR |
| client_name : STR |
| client_visible: BOOL| ← デフォルト false
| unit_price  : INT |
...
| client_visible | boolean | YES | false | クライアント名を公開するか |

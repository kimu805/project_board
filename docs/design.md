...
| 12 | 参画ロール・月給計算 | 案件登録・編集・詳細画面 | 案件ごとに「メンバー」または「リーダー」のロールを保持し、単価をもとに月給を算出・表示する |
...
- 案件カードに以下を表示：案件名、クライアント名（表示ON時のみ）、単価、月給（ロールに応じた計算値）、勤務形態、期間、技術スタック
- 参画期間の横に期間の長さを表示する（例：`2023/04 〜 2024/01（8.5ヶ月）`）
  - 終了日が未設定の場合は今日を終端として計算
  - 12ヶ月未満は `Nヶ月`（小数点第1位まで）、12ヶ月以上は `N年Nヶ月` 形式
  - 1年ちょうど・2年ちょうどなど端数なしの場合は `N年` のみ表示
...
- 左カラム：案件名、クライアント名、技術スタック、参画期間
  - 参画期間の横に期間の長さを表示する（例：`2023/04 〜 2024/01（8.5ヶ月）`）
  - 一覧画面と同じ計算ルール（終了日未設定は今日を終端、12ヶ月以上は年単位）
- 右カラム：月額単価、参画ロール（メンバー / リーダー）、月給（ロールに応じた計算値）、勤務形態、操作ボタン（編集・削除）
...
- ロール選択欄：「メンバー」「リーダー」をラジオボタンまたはセレクトボックスで選択（必須）
...
案件ごとに自分の参画ロールを「メンバー」または「リーダー」で記録し、月額単価とロールをもとに月給（手取り見積もりの参考値）を自動計算・表示する。

### ロール定義と月給計算式

| ロール | キー | 月給計算式 |
|--------|------|------------|
| メンバー | `member` | 月給 = 月額単価 × 0.65 |
| リーダー | `leader` | 月給 = 月額単価 × 0.70 |

### データモデル

`projects` テーブルに `role` カラム（integer 型、NOT NULL）を追加し、enum で管理する。
...
表示名は `config/locales/ja.yml` の `enums.project.role` で管理する。
...
|------|----------|
| 参画ロール | 「メンバー」または「リーダー」 |
| 月給 | `¥NNN,NNN`（`number_to_currency` で整形） |

#### 案件一覧画面（index）

案件カードに月給を表示する。単価の直下など視認しやすい位置に配置し、ロール名を括弧で添える。

表示例：`¥455,000（メンバー）`

#### 案件登録・編集画面（new / edit）

ロール選択欄をセレクトボックスで実装する。デフォルトは「メンバー」。

```erb
<%= f.select :role, Project.roles.keys.map { |k| [I18n.t("enums.project.role.#{k}"), k] }, {}, class: "..." %>
```

### バリデーション追加

`role` は必須入力とする。`enum :role, ..., validate: true` により無効値はバリデーションエラーになる。

### Strong Parameters 更新

`projects_controller.rb` の `project_params` に `:role` を追加する。
